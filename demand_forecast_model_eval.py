...
Created in 2018
@author: aszewczyk
Function that evaluates model errors (AE and APE) on flight, day, month, month-segment levels
Input:
...
def demand_forecast_model_eval(model_results_spk,
                               col_target,
                               col_predict,
                               model_evaluation,
                               eval_start,
    ...
    try:
        ...
        if verbose:
            ...
            logger.info('Outliers (APE on flight level > 50%)) part is ' +
                  str(round(model_results_spk.filter(col(col_target+'_APE') > 50).count()*100.00/model_results_spk.count(), 2)) + '%')
            logger.info('Outliers (APE on flight level > 100%)) part is ' +
                  str(round(model_results_spk.filter(col(col_target+'_APE') > 100).count()*100.00/model_results_spk.count(), 2)) + '%')
        if model_evaluation == 'full':
            agg_day_segment = model_results_spk.groupBy('dt_flight_date_local', 'cd_airport_pair')\
                                   .agg({col_target: 'sum', col_predict: 'sum'})
            agg_day_segment = agg_day_segment.select('*', abs(agg_day_segment['sum('+col_target+')'] - agg_day_segment['sum('+col_predict+')'])\
                                       .alias('daily_abs_error'))
            agg_day_segment = agg_day_segment.select('*', abs((agg_day_segment['sum('+col_target+')'] - agg_day_segment['sum('+col_predict+')'])
                                               /agg_day_segment['sum('+col_target+')']*100)\
                               .alias('daily_abs_perc_error'))
            if verbose:
                logger.info('MAE agg on day-airport pair level is ' +
                         str(round(agg_day_segment.agg({'daily_abs_error': 'avg'}).collect()[0][0], 3)))
                logger.info('MAPE agg on day-airport pair level is ' +
                         str(round(agg_day_segment.agg({'daily_abs_perc_error': 'avg'}).collect()[0][0], 3)) + ' %')

        #evaluation on month-segment level
        agg_month_segment = model_results_spk.groupBy('dt_flight_year_month', 'cd_airport_pair_order')\
                         .agg({col_target: 'sum', col_predict: 'sum'})
        agg_month_segment = agg_month_segment.select('*', abs(agg_month_segment['sum('+col_target+')'] - agg_month_segment['sum('+col_predict+')'])\
        ...
        if verbose:
            logger.info('MAE agg on month-airport pair order level is ' +
                     str(round(agg_month_segment.agg({'monthly_abs_error': 'avg'}).collect()[0][0], 3)))


            logger.info('MAPE agg on month-airport pair order  level is ' +
                     str(round(agg_month_segment.agg({'monthly_abs_perc_error': 'avg'}).collect()[0][0], 3)) + ' %')
        #calcualte extra evaluation metrics
        if model_evaluation == 'full':
            #evaluation on day level
            agg_day = model_results_spk.groupBy('dt_flight_date_local')\
                       .agg({col_target: 'sum', col_predict: 'sum'})
            ...
            agg_month = model_results_spk.groupBy('dt_flight_year_month')\
                             .agg({col_target: 'sum', col_predict: 'sum'})
