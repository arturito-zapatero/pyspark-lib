...
from lib.calc_feat_importance_all import calc_feat_importance_all
from lib.calc_prediction_error import calc_prediction_error
from lib.create_model_data_lists import create_test_data_lists
from lib.create_spark_session import create_spark_session
from lib.create_test_dates_list import create_test_dates_list
from lib.define_features import define_features
from lib.rf_feature_importance import rf_feature_importance
...
def demand_forecast_pred(data,
    ...
    try:
        ...
        include_past_flights_data=parser.getboolean('save_params', 'include_past_flights_data') 
        local_save_path = parser.get('save_params', 'local_save_path')
        #define name of the variable for predictions
        col_predict = col_target + '_pred'
        model_complexity='fourth'
        use_clustered_data_sets=False
        include_past_flights_data=False
        ...
        #should implement algo to find dt_execution for the last run model here!!
        if not dt_execution:
        ...
        #connect to spark s.
        spark = create_spark_session(jarra='mass',
                                     verbose=True,
                                     logger=logger)
        ...
        for i in range(number_of_models):
            ...
            pipelinePrepBasicList.append(PipelineModel.read().load(s3_save_pipelines_path+"data_prep_basic_pipeline"+str(i)))
            fitList.append(RandomForestRegressionModel.read().load(s3_save_pipelines_path+"model_pipeline"+str(i)))
            fitBasicList.append(RandomForestRegressionModel.read().load(s3_save_pipelines_path+"model_basic_pipeline"+str(i)))
        if verbose:
        ...
        start_days_list, end_days_list = create_test_dates_list(split_value,
                                                                verbose=verbose,
                                                                logger=logger
                                                                )
        ...
        #create list with test data sets for each of the consecutive models, each data set have different features and dates
        #also data list for rows/flights with Nulls (e.g. no historical data) is created separately
        test_data_list, test_data_basic_list = create_test_data_lists(data,
                                     features_list,
                                     ...
                                     verbose,
                                     logger)
        #ad-hoc, there are some nans in hour (should not be in the new table ds_flights)
        for i in range(len(test_data_list)):
            test_data_list[i] = test_data_list[i].dropna(how='any', subset=['dt_flight_hour_local_zone_sin'])
            test_data_basic_list[i] = test_data_basic_list[i].dropna(how='any', subset=['dt_flight_hour_local_zone_sin'])    
        ...
        for i in range(len(test_data_list)):
            ...
            #features_list[i] = features_list[i] + ohe_variables_out #delete? si!
            ...
            testDataBasicList.append(pipelinePrepBasicList[i].transform(test_data_basic_list[i]))
        if verbose:
        ...
        for i in range(len(testDataList)):
            #use the test set, is creatring an extra column 'col_target' with the test fit results
            resultsList.append(fitList[i].transform(testDataList[i]).select(cols_id + [col_predict]))
            #the same for 'basic' models
            resultsBasicList.append(fitBasicList[i].transform(testDataBasicList[i]).select(cols_id + [col_predict]))
            end = datetime.now()
        if verbose:
        ...
        featureImportancesFirst, featureImportancesLast, feature_importances_all = calc_feat_importance_all(fitList,
                                                                                   testDataList,
                                                                                   col_target,
                                                                                   first_pred_day,
                                                                                   verbose,
        ...
        if verbose and checks:
            df_prediction_errors, pred_errors = calc_prediction_error(number_of_last_days_to_eval=90,
            ...
            #for i in range(len(testDataList)):
            #    testSetCount = 0
            #    testSetCount += testDataList[i].count()
            ...
            #testSetCount = testDataList[0].count() + testDataList[1].count() + testDataList[2].count() +\
            #               testDataList[3].count() + testDataList[4].count() + testDataList[5].count()
            #testBasicSetCount = testDataBasicList[0].count() + testDataBasicList[1].count() + testDataBasicList[2].count() +\
            #                    testDataBasicList[3].count() + testDataBasicList[4].count() + testDataBasicList[5].count()
